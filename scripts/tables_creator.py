...
def parse_bool(x):
    ...
    if pd.isna(x):
        return None
    s = str(x).strip().lower()
    if s in {"true", "1", "yes", "y", "այո"}:
        return True
    if s in {"false", "0", "no", "n", "ոչ"}:
        return False
    return None
...
if "url" not in df.columns:
    raise ValueError("CSV-ում 'url' սյուն չկա։ Ստուգիր քո ֆայլի header-ը։")
...
# =======================
print("Պատրաստ է ✅")
print(f"Տողերի քանակը (unique listings): {len(df)}")
print("Ֆայլերը պահպանվել են այստեղ 👇")
